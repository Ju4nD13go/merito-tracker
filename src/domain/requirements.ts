...
import type { Requisitos, Vacancy } from './contracts.ts'
...
export function parseExperienceYears(text: string | null): number | null {
  ...
  if (/no inferior a/.test(normalized)) {
    ...
    // fallback: the number before "años"
    const n = normalized.match(/no inferior a\s+(\d+(?:\.\d+)?)/)
...
export function detectDisciplines(text: string): string[] {
  ...
  return disciplines.filter((d) => t.includes(d))
}
...
function checkVacancyRequirements(v: Vacancy, p: UserProfile): RequirementCheck {
  ...
  const expText = v.requisitos.experiencia ?? extractExperienceText(estudio)
  const requiredYears = parseExperienceYears(expText ?? '')
...
export function describeRequirements(v: Vacancy): RequirementProfile {
  ...
  return {
    ...
    requiredYears: parseExperienceYears(expText ?? ''),
